...
int main() {
 while (FCGI_Accept() >= 0) {
    ...
    json_array_foreach(root, index, value) {
        ...
        id_number = atoi(json_string_value (json_object_get(value, "value"))); // atoi convert string to int
        
       result = strcmp(name, "id_number");
    ...
    rc = sqlite3_open("db/finals.db", &db);
    ...
    const char *sql = id_number == 0 ? "SELECT * FROM info WHERE id_number > ?" : "SELECT * FROM info WHERE id_number = ?";
