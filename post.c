...
void create_json_array(Report info) {
    ...
    json_object_set_new(obj, "id_number", json_string(info.id_number));
...
void loadJSON(Report *info) {  
    ...
    const char* properties[] = { "id_number", "first_name", "last_name", "date", "status", "year", "section" };
    char** infovar[] = {&info->id_number,&info->first_name,&info->last_name,&info->date,&info->status,&info->year,&info->section};
...
int main(void) {
  ...
  while (FCGI_Accept() >= 0) {
   ...
   loadJSON(&info);
    ...
    dbres = sqlite3_open_v2("db/finals.db", &db, SQLITE_OPEN_READWRITE,NULL);
    ...
    dbres = sqlite3_bind_text(stmt, 1, info.id_number, -1, SQLITE_TRANSIENT);
    // bitwise operator
    dbres |= sqlite3_bind_text(stmt, 2, info.first_name, -1, SQLITE_TRANSIENT); 
    dbres |= sqlite3_bind_text(stmt, 3, info.last_name, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 4, info.date, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 5, info.status, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 6, info.year, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 7, info.section, -1, SQLITE_TRANSIENT);
